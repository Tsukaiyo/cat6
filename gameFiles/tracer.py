...
This is an original, multiplayer arcade game featuring both competitve and co-op modes for 2-4 players.
It can run both on desktop computers and raspberry pi arcade machines, controlled by joysticks. If not enough joysticks are availible, it uses arrow keys, WASD, TFGH, and IJKL.  
'''
    ...
    if targetOutOfRange or closeToTarget:  # pick new direction
        ...
        for dots in animationDots:
            for i in range(len(dots)):
                if dist(dots[i], aniRocket) < 5 and not dots[
                                                            i].colour == aniRocket.colour:  # Check if touching a dot of a different colour
                    touchingDot = True
...
def drawButtons(selected):
    ...
    if selected >= 0:
        arrowPos = (
        (buttons[selected].x - 15, buttons[selected].y + 25), (buttons[selected].x - 45, buttons[selected].y + 35),
...
# Core methods
def render(gameState, winner, endTime, endOfRound, startTime, players, team, grid, turnList, currentSquare, mazeCreated,
           buttonsMade, selected, maze, points, lives, explosions, name, selectedLetter, scoreWritten):
            ...
            for x in range(size):
                for y in range(size):
                    for rocket in rockets:
                        ...
                        if grid[x][y].nWall:
                            if (y * Height / size) - 5 < rocket.y and (
                                    y * Height / size) + 5 > rocket.y and x * Width / size < rocket.x and (
                                    x + 1) * Width / size > rocket.x:
                        ...
                        if grid[x][y].eWall:
                            if ((x + 1) * Width / size) - 5 < rocket.x and (
                                    (x + 1) * Width / size) + 5 > rocket.x and y * Height / size < rocket.y and (
                                    y + 1) * Height / size > rocket.y:
                                lives = lifeLost(lives, rocket, explosions, explosionStarted)[0]
                                explosions = lifeLost(lives, rocket, explosions, explosionStarted)[1]
                                explosionStarted = True
                        if grid[x][y].sWall:
                            if ((y + 1) * Height / size) - 5 < rocket.y and (
                                    (y + 1) * Height / size) + 5 > rocket.y and x * Width / size < rocket.x and (
                                    x + 1) * Width / size > rocket.x:
                                lives = lifeLost(lives, rocket, explosions, explosionStarted)[0]
                                explosions = lifeLost(lives, rocket, explosions, explosionStarted)[1]
                                explosionStarted = True
                        if grid[x][y].wWall:
                            if (x * Width / size) - 5 < rocket.x and (
                                    x * Width / size) + 5 > rocket.x and y * Height / size < rocket.y and (
                                    y + 1) * Height / size > rocket.y:
                                lives = lifeLost(lives, rocket, explosions, explosionStarted)[0]
                                explosions = lifeLost(lives, rocket, explosions, explosionStarted)[1]
                                explosionStarted = True
...
def draw(gameState, winner, grid, size, mazeCreated, selected, points, lives, gameTime, startTime,
    ...
    if gameState == "Instructions":
        ...
        text("Competitive mode: Use your trail to cut off another player", 'impact', 30, RED2, 100, 375)
    ...
    if gameState == "ToggleMaze":
        ...
        if len(buttons) > 0:
            drawButtons(selected)
        text("Select Game Mode", 'impact', 100, PURPLE, 100, 245)
        text("Select Game Mode", 'impact', 100, TEAL, 105, 250)
    if gameState == "Play":
        screen.fill(BLUE)
        for rocket in rockets:
...
while not done:
    ...
    # update game info
    info = render(gameState, winner, endTime, endOfRound, startTime, players, team, grid, turnList, currentSquare,
                  mazeCreated, buttonsMade, selected, maze, points, lives, explosions, name, selectedLetter,
                  scoreWritten)
    gameState = info[0]
    winner = info[1]
    players = info[5]
