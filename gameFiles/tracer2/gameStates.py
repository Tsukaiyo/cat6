...
class EndMaze(End):
    ...
    def playerNameInput(self):
        ...
        if key[pygame.K_DOWN]:
            if self.selectedLetter < len(self.letters) - 1:
                self.selectedLetter = self.selectedLetter + 1
            else:
                self.selectedLetter = 0
        if key[pygame.K_UP]:
            if self.selectedLetter <= 0:
                self.selectedLetter = len(self.letters) - 1
            else:
                self.selectedLetter = self.selectedLetter - 1
        if key[pygame.K_SPACE]:
            self.name = self.name + self.letters[self.selectedLetter]
            self.selectedLetter = 0
